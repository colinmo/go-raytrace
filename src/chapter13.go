...
func ChapterThirteen() {
	...
	left.SetMinimum(-1)
	left.SetMaximum(1)
	left.SetTransform(NewTranslation(-1.5, 1, 1.75))
	left.SetTransform(left.Transform.MultiplyMatrix(NewScaling(0.20, 0.20, 0.20)))
	...
	tempFile := filepath.Join(os.TempDir(), "chapter13.ppm")
