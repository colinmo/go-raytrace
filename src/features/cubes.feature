Feature: Cubes
    ...
    Scenario Outline: A ray intersects a cube
        Given shapes.c ← cube()
        And ray.r ← ray(<origin>, <direction>)
        When arrayintersections.xs ← local_intersect(shapes.c, ray.r)
        Then arrayintersections.xs.count = 2
        And arrayintersections.xs[0].t = <t1>
        And arrayintersections.xs[1].t = <t2>
        Examples:
    ...
    Scenario Outline: The normal on the surface of a cube
        Given shapes.c ← cube()
        And tuple.p ← <point>
        When tuple.normal ← local_normal_at(shapes.c, tuple.p)
        Then tuple.normal = <normal>
        Examples:
            | point                | normal           |
